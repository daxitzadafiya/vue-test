<template>
  <!-- we can alos use thr create form to update data for this we need to set up the form conditionally -->
  <b-row>
    <h3><strong>Update user details</strong></h3>
    <b-col></b-col>
    <b-col
      ><div class="container">
...
export default {
  ...
  async created() {
    ...
    const data = await registrationRepository.getUser(id);
    console.log(data);
  ...
  methods: {
    async onSubmit() {
      if (
        ...
        try {
          const id = this.$route.params.id;
          await registrationRepository.update(this.formData, id);
          this.$swal("record updated successfully");
          this.$router.push("/");
        } catch (error) {
          console.log(error);
